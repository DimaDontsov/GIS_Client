...
adminMode = new Vue({
  ...
  methods: {
    ...
    login: function() {
      ...
      $.get(
        ...
        function(data) {
          ...
          if (isAuth === true) {
            t.isLogged = true;
            localStorage.setItem("login", t.user.login);
            localStorage.setItem("password", t.user.password);
            localStorage.setItem("isLogged", t.isLogged);
            $("#myModal").modal("hide");
          } else {
            t.isLogged = false;
    ...
    drawNewSegment: function() {
      ...
      this.newLinkCanv.clearRect(
        0,
        0,
        this.newLinkCanv.canvas.width,
        this.newLinkCanv.canvas.height
      );
        ...
        $.get(
          ...
          function(data) {
            if (data.status !== "Error") {
              ...
              allert(
                "Простите, сервер перестал нас слушаться. Он решил, что он умнее ;("
              );
            }
          },
          "Json"
...
function onWhatAddressThereAnswer(data, textStatus, jqXHR) {
  ...
  if (
    getFirstWayPointToWayInfo() == null ||
    getFirstWayPointToWayInfo() == ""
  ) {
    setFirstWayPointToWayInfo(addr === "" || addr === null ? "------" : addr);
  } else if (
    getSecondWayPointToWayInfo() == null ||
    getSecondWayPointToWayInfo() == ""
  ) {
...
function onPointMenuClick(event) {
  ...
  if (selectedIndex === 0) {
    showWayInfo();
    if ($(".way-point-icon").length >= 2) removeWayPointIcons();
